...
import { Sparkles, Target, Brain, Search, RefreshCw, Filter, Zap, Users, TrendingUp } from "lucide-react";
...
export default function MatchesPage() {
  ...
  const filterOptions = [
    { key: "all", label: "All Matches", icon: Users, count: () => matches.length },
    { key: "current", label: "Active", icon: Zap, count: () => matches.filter(m => m.status === "accepted").length },
    { key: "pending-sent", label: "Sent", icon: TrendingUp, count: () => matches.filter(m => m.status === "pending" && m.requester._id === userId).length },
    { key: "pending-received", label: "Received", icon: Target, count: () => matches.filter(m => m.status === "pending" && m.receiver._id === userId).length },
    { key: "rejected", label: "Declined", icon: Filter, count: () => matches.filter(m => m.status === "rejected").length },
  ];
    ...
    <div className="relative overflow-hidden min-h-screen bg-gradient-to-br from-black via-gray-950 to-slate-950">
      ...
      <div className="relative z-10 p-6 lg:p-8">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-r from-emerald-400 via-green-500 to-teal-600 rounded-3xl mb-6 shadow-2xl shadow-emerald-500/30 relative overflow-hidden group">
          ...
          <p className="text-xl text-slate-300 mb-6 max-w-3xl mx-auto leading-relaxed">
            Discover compatible learning partners, manage your connections, and unlock your potential through intelligent skill matching
          </p>
          ...
          <div className="flex items-center justify-center space-x-6 mb-8 flex-wrap">
            <div className="flex items-center space-x-2 bg-gradient-to-r from-emerald-400/10 via-green-500/10 to-teal-600/10 backdrop-blur-sm rounded-full px-4 py-2 border border-emerald-400/20">
              ...
              <span className="text-emerald-400 font-semibold">Live Network</span>
            </div>
            <div className="flex items-center space-x-2 bg-gradient-to-r from-green-400/10 via-teal-500/10 to-emerald-600/10 backdrop-blur-sm rounded-full px-4 py-2 border border-green-400/20">
              ...
              <span className="text-green-400 font-semibold">Smart AI Matching</span>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto mb-12">
          <div className="relative group">
            ...
            <input
              ...
              className={`w-full pl-16 ${searchQuery ? 'pr-16' : 'pr-6'} py-4 bg-gray-800/50 backdrop-blur-sm border border-slate-600/30 rounded-3xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400/50 transition-all duration-300 hover:bg-gray-800/70 text-lg`}
            />
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-gray-900/90 via-gray-950/95 to-black/90 backdrop-blur-xl rounded-3xl border border-slate-800/40 p-2 mb-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex space-x-2 relative z-10">
            {[
              { id: "requests", icon: Target, label: "Match Requests", count: stats.requests },
              { id: "smart", icon: Brain, label: "Smart Matches", count: stats.smart, special: true }
            ].map((tab) => (
              <button
                key={tab.id}
              ...
              >
                ...
                <div className="relative z-10 flex items-center space-x-3">
                  <div className={`transition-all duration-300 ${activeTab === tab.id ? 'scale-110 rotate-12' : 'group-hover:scale-110 group-hover:rotate-6'}`}>
                    <tab.icon className={`w-5 h-5 ${activeTab === tab.id ? 'text-white drop-shadow-lg' : 'text-slate-400 group-hover:text-emerald-400'}`} />
                  </div>
                  <span className={`${activeTab === tab.id ? 'drop-shadow-lg' : ''}`}>{tab.label}</span>
                  {tab.special && <Sparkles className={`w-4 h-4 ${activeTab === tab.id ? 'text-yellow-300' : 'text-yellow-400'} animate-pulse`} />}
                  {tab.count > 0 && (
            ...
            {activeTab === "smart" && (
              <button
                ...
                className="px-6 py-4 bg-gradient-to-r from-emerald-600 via-green-600 to-teal-600 hover:from-emerald-700 hover:via-green-700 hover:to-teal-700 text-white rounded-2xl font-semibold transition-all duration-300 transform hover:scale-105 shadow-2xl shadow-emerald-500/40 hover:shadow-emerald-500/60 disabled:opacity-50 relative overflow-hidden group border border-emerald-400/20"
              >
                ...
                <div className="relative z-10 flex items-center space-x-2">
                  <RefreshCw className={`w-4 h-4 ${smartMatchesLoading ? "animate-spin" : "group-hover:rotate-180 transition-transform duration-500"}`} />
                  <span>Refresh</span>
                </div>
              </button>
            )}
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-8">
                  <h3 className="text-2xl font-bold text-white flex items-center space-x-3">
                    ...
                    <span className="bg-gradient-to-r from-emerald-300 via-green-400 to-teal-400 bg-clip-text text-transparent">Filter Matches</span>
                  </h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                  {filterOptions.map((option, index) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`absolute inset-0 bg-gradient-to-r from-emerald-500/10 via-green-500/15 to-teal-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl ${isActive ? 'opacity-100' : ''}`}></div>
                        ...
                        <div className="relative z-10 text-center">
                          ...
                          <div className={`text-2xl font-bold ${isActive ? 'text-emerald-300' : 'text-slate-300 group-hover:text-emerald-300'} transition-colors duration-300`}>
                            {count}
                          </div>
                        </div>
                        
                        {/* Hover effect line */}
                        <div className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-emerald-400 via-green-500 to-teal-600 rounded-b-2xl transition-all duration-300 ${
            ...
            {loading ? (
              <div className="flex justify-center items-center h-64">
                <div className="text-center">
                  ...
                  <p className="text-slate-400 mt-2">Finding your perfect learning partners</p>
                </div>
              </div>
                ...
                <div className="flex items-center space-x-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-slate-300 mt-1">
                      Our intelligent algorithm analyzes your skills, preferences, and learning goals
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                  <div className="bg-gradient-to-r from-emerald-500/10 to-green-500/10 rounded-xl p-4 border border-emerald-500/20">
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      <span className="text-emerald-400 font-semibold">Compatibility Score</span>
                    </div>
                    <p className="text-slate-300 text-sm">Advanced matching algorithms calculate perfect partnerships</p>
                  </div>
                  <div className="bg-gradient-to-r from-blue-500/10 to-teal-500/10 rounded-xl p-4 border border-blue-500/20">
                    <div className="flex items-center space-x-2 mb-2">
                      <Target className="w-4 h-4 text-blue-400" />
                      <span className="text-blue-400 font-semibold">Skill Alignment</span>
                    </div>
                    <p className="text-slate-300 text-sm">Match with users who complement your learning goals</p>
                  </div>
                  <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-xl p-4 border border-purple-500/20">
                    <div className="flex items-center space-x-2 mb-2">
                      <Users className="w-4 h-4 text-purple-400" />
                      <span className="text-purple-400 font-semibold">Mutual Learning</span>
                    </div>
                    <p className="text-slate-300 text-sm">Find partners for reciprocal skill exchange</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {smartMatchesLoading ? (
              <div className="flex justify-center items-center h-64">
                <div className="text-center">
                  ...
                  <p className="text-slate-400 mt-2 leading-relaxed">Finding your most compatible learning partners</p>
            ...
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {[
                  ...
                  { 
                    label: 'Success Rate', 
                    value: stats.current > 0 ? Math.round((stats.current / stats.requests) * 100) + '%' : '0%', 
                    icon: TrendingUp,
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="relative z-10">
                        <div className={`inline-flex items-center justify-center w-12 h-12 bg-gradient-to-r ${stat.color} rounded-xl mb-3 shadow-lg transform group-hover:scale-110 transition-transform duration-300`}>
